...
import { 
  Package, Users, Ticket, TrendingUp, Layers, ArrowUpRight, 
  FileText, PenTool, ShoppingCart, ClipboardList, Users2, 
  HeartHandshake, Truck, Palette, Calculator 
} from "lucide-react";

// Mapping für die Icons im Modul-Bereich
...
export default async function AdminDashboard() {
  ...
  // AUTH-UPLINK: Wir holen den echten User vom Server
  const { data: { user } } = await supabase.auth.getUser();
  
  // Wir priorisieren den Namen aus den Metadaten, sonst Email, sonst "Operator"
  const displayName = user?.user_metadata?.full_name || user?.email?.split('@')[0] || "Operator";
  const userEmail = user?.email;

  // 1. LIVE-DATEN ABFRAGEN
  ...
  // 2. EINSTELLUNGEN & MODULE LADEN (Dynamisch via Email des aktuellen Users)
  ...
  return (
    <div className="p-8 space-y-10 font-sans bg-black min-h-screen text-white">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase italic">
            {/* DYNAMISCHER NAME [cite: 2026-03-28] */}
            {greeting}, {displayName} 👋
          </h1>
          <p className="text-slate-500 text-[10px] mt-2 uppercase tracking-[0.2em] font-bold">
            System-Status: Optimal // AETHER OS Operativ // ID: {user?.id.slice(0,8)}
          </p>
        </div>
      ...
      {/* Stats Grid - Unverändert, aber jetzt mit echten DB-Werten befeuert */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="p-8 bg-zinc-950/50 border border-white/5 rounded-[2.5rem] shadow-2xl relative overflow-hidden group hover:border-blue-500/30 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-zinc-950/40 border border-white/5 rounded-[2.5rem] p-10">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-[11px] font-black text-white italic uppercase tracking-[0.3em]">Aktive Module & Schnittstellen</h2>
          </div>
          <div className="flex flex-wrap gap-4">
            {activeModuleKeys.length > 0 ? (
              activeModuleKeys.map((key: string) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-3 px-6 py-3.5 bg-zinc-900/80 border border-white/5 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white hover:border-blue-500/50 transition-all cursor-default">
              ...
              <p className="text-slate-600 text-[10px] italic uppercase tracking-widest py-2">Keine Module via DB aktiviert.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-[#b33927] rounded-[2.5rem] p-10 text-white relative overflow-hidden flex flex-col justify-between group cursor-pointer shadow-2xl shadow-red-950/20 active:scale-[0.98] transition-transform">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60">Aether OS Core</p>
            <h2 className="text-2xl font-black mt-4 leading-tight uppercase italic tracking-tighter">Konfiguration <br />erweitern?</h2>
          </div>
          <button className="mt-10 bg-black text-white text-[10px] font-black py-4 rounded-2xl uppercase tracking-[0.2em] hover:bg-zinc-900 transition-all shadow-2xl">Zum Modulstore</button>
        </div>
      </div>
    </div>
  );
